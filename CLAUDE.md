...
- **PII guard is automated.** `scripts/check_pii.py` (stdlib-only) scans
  `oteru-emitter/samples/`, `oteru-emitter/tests/fixtures/` and the OTLP
  payloads generated by `oteru-emitter/tests/factories.py` for real e-mails,
  ...
  and as a CI job. New fixtures — committed or generated — must stay clean
  under it.
- **Signals are independent, and selection is not generation.** The emitter's
  ...
  `OTEL_TRACES_EXPORTER=otlp`). The committed sample has no traces because it
  was captured with the beta off, not because that version lacked support. New
  captures are never committed — the redacted historical sample is the one
  exception; the trace path is exercised by an OTLP payload built in
  `oteru-emitter/tests/factories.py`.
